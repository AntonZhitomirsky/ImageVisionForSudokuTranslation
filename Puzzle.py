...
class Puzzle:
    ...
    def normalise_image(self, corners=None):
        # finding corners
        if corners is None:
            findCorners = Corner_Detection.Corner(matrix=self.__puzzle_square.get_matrix(), step=1, start=1,
                                              threshold_for_complete=5, filter_size=17)

            if findCorners.has_corners() is False:
                raise Exception('no corners were found in this image')

            corners = findCorners.get_corners()
